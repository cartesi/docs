...
The [CartesiDApp](https://github.com/cartesi/rollups/tree/main/onchain/rollups/contracts/dapp/CartesiDApp.sol) contract acts as the base layer incarnation of a DApp running on the execution layer. The DApp is hereby able to interact with other smart contracts through the execution of [vouchers](../../components.md#vouchers)
...
A **notice**, on the other hand, constitutes an arbitrary piece of data that can be proven any number of times. On their own, they do not trigger any type of contract-to-contract interaction.
...
The consensus has the power of submitting claims, which, in turn, are used to validate vouchers and notices. Meanwhile, the owner has complete power over the DApp, as it can replace the consensus at any time. Therefore, the users of a DApp must trust both the consensus and the DApp owner.
...
| ---- | ---- | ----------- |
| _destination | address | The message call destination address |
| _payload | bytes | The message call payload |
