...
* [**Cartesi Rollups**](../cartesi-rollups/components.md), a set of on-chain and off-chain components that implement an Optimistic Rollups solution and provide the general framework for building DApps.
* [**Cartesi Machine**](../machine/intro.md), a virtual machine (VM) that runs an entire Linux OS, in which each DApp's [back-end](../cartesi-rollups/dapp-architecture.md#back-end) is executed.
...
In the next sections, we will first describe [Cartesi Rollups' internal components](../cartesi-rollups/components.md) and how they work. Then, we will explain in detail the [architecture of DApps](../cartesi-rollups/dapp-architecture.md) that run on our rollups framework, as well as the [APIs that developers can use](../cartesi-rollups/http-api.md) to communicate with it. Finally, we share our vision of a [step-by-step process for developing applications](../cartesi-rollups/dapp-life-cycle.md) on Cartesi Rollups, from the initial design stage up to its final deployment.
