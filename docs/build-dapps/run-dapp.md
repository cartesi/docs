---
id: run-dapp
title: Run your first DApp
---

In this section, we will explain how to run a [Simple Echo DApp](https://github.com/cartesi/rollups-examples/tree/main/echo) example that shows how to build and interact with a minimalistic Cartesi Rollups application that simply copies (or "echoes") each input received as a corresponding output notice. The DApp's back-end is written in Python, and its front-end is simply a set of standard Hardhat command line tasks that can be executed from a terminal.

Before getting started, make sure you have [installed all the necessary requirements](../requirements).

:::note
Learn more: [Cartesi Rollups](../../cartesi-rollups/overview/), [DApp architecture](../../cartesi-rollups/dapp-architecture/), the [HTTP API](../../cartesi-rollups/http-api/), and the [DApp Life Cycle](../../cartesi-rollups/dapp-life-cycle/).
:::
...
When developing an application, it is often important to easily test and debug it. For that matter, it is possible to run the Cartesi Rollups environment in [host mode](../overview#host-mode), so that the DApp's back-end can be executed directly on the host machine, allowing it to be debugged using regular development tools such as an IDE.
