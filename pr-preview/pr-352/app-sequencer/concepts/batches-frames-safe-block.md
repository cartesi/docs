...
While the sequencer is filling a batch, the base layer keeps producing blocks and direct inputs keep arriving. Frames let one batch record that some transactions were evaluated at block 95 and later transactions at block 100. The batch can therefore remain open while the sequencer's base-layer view advances.
...
```
Batch, nonce 42
  frame 1   safe block 95  transactions A, B, C
  frame 2   safe block 100 transaction D
```

Run by the machine, that becomes:

```
run direct inputs recorded up to block 95
run A, B, C
