...
Crucially, the base layer conducts on-chain validation of these notices through the [`validateNotice()`](../base-layer-contracts/   application.md/#validatenotice) function of the `CartesiDApp` contract.
...
Frontend clients can query notices using a GraphQL API exposed by the Cartesi Nodes. [Refer to the documentation here](../../build/query-outputs) to query notices from the rollup server.
:::
